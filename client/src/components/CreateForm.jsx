...
const CreateForm = ({ updateMessage }) => {
  ...
  const sendNewSpecies = (data) => {
    data.pollinators = selectedPollinators;
    data.common_names = commonFields;
    
    if (authService.isSignedIn()) {
      speciesService.createSpecies(data, (isSuccessful, response) => {
        if (isSuccessful) {
          navigate('/');
  ...
  return (
    <form onSubmit={handleSubmit(sendNewSpecies)} className="form-create">
      <h1 className="h3 mb-3 font-weight-normal text-center">Add New Species</h1>
